...
const processForm = reactive({
  remark: '',
  newStatus: 'COMPLETED' as ServiceOrderStatus,
})
...
function renderCharts() {
  ...
  filtered.value.forEach((item) => {
    const m = dayjs(item.appointmentTime).format('MM月')
...
<template>
  <el-card class="page-card">
    ...
    <el-table v-loading="loading" :data="filtered" border height="500">
      ...
      <el-table-column label="派单方" min-width="100">
        <template #default="{ row }">{{ row.dispatchedBy || '-' }}</template>
      </el-table-column>
      <el-table-column label="需求描述" min-width="200" show-overflow-tooltip>
        <template #default="{ row }">{{ row.requirement || row.remark || '-' }}</template>
      </el-table-column>
